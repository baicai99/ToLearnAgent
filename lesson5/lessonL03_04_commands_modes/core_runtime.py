# -*- coding: utf-8 -*-
"""
标题：L03-04 核心运行时（沙箱 + 读/搜/写 + 补丁）
执行：python -m lessonL03_04_commands_modes.main

[L03-04 NEW]
- 保持 L03-03 的稳定性：list_files 空字符串->".", read_file_range clamp, grep 内置
"""
...
class Runtime:
    ...
    def read_file_range(self, path: str, start_line: int = 1, end_line: int = 200) -> Dict[str, Any]:
        ...
        if total == 0:
            return {"path": path, "total_lines": 0, "start_line": 1, "end_line": 1, "text": ""}

        sl = min(sl, total)
        el = min(el, total)

        snippet = "\n".join(lines[sl - 1 : el])
        return {"path": path, "total_lines": total, "start_line": sl, "end_line": el, "text": snippet}

    def grep(self, pattern: str, path: str | None = ".", max_results: int = 50) -> List[Dict[str, Any]]:
...
def apply_hunks_to_text(old_text: str, hunks: List[Dict[str, Any]]) -> str:
    ...
    hunks_sorted = sorted(hunks, key=lambda x: (int(x.get("start_line", 0)), int(x.get("end_line", 0))))
