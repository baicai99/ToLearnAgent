# -*- coding: utf-8 -*-
"""
标题：L02-08A core_runtime.py —— 任务路由 + 工具契约（核心运行时）
执行代码：
...
def set_turn_context(user_text: str) -> TaskType:
    """
    每轮任务路由：review/create/tests/implement
    - review：查看目录/结构/审阅（默认）
    - create：创建/新建/生成文件
    - tests：修复测试、pytest、测试通过
    - implement：修改/修复/新增功能（非明确测试）
    """
    _require_init()
...
# -----------------------------
# 核心能力：写入（create / implement / tests）
# -----------------------------
    ...
    if STATE.write_policy == "ask":
        ...
        if ans not in ("y", "yes"):
            ...
            return _json(
                ...
                file=rel,
                next="ASK_USER",
                message="用户拒绝了写入；不要重复同一写入，应询问用户希望如何处理。",
            )
...
def apply_hunks_impl(file_path: Optional[str], hunks: Any) -> str:
    ...
    # 关键：这里不依赖 Pydantic 结构校验，防止模型乱传导致程序崩溃
    if (
    ...
    if STATE.write_policy == "ask":
        ...
        if ans not in ("y", "yes"):
            ...
            return _json(
                ...
                file=rel,
                next="ASK_USER",
                message="用户拒绝了补丁；不要重复同一修改，应询问用户希望如何处理。",
            )
...
# -----------------------------
# 核心能力：bash / evidence / todo / submit
# -----------------------------
...
def submit_impl(final: Optional[str], evidence: Any, task_type: Optional[str]) -> str:
    ...
    if task_type in ("review", "create", "tests", "implement"):
        tt = task_type  # 允许模型显式指定，但仍会按规则校验
