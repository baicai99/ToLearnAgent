...
标题：L03-02 工具层（LangChain @tool）

[L03-02 NEW]
- 所有工具均为 @tool，并具备 docstring（避免“无 docstring”报错）
- propose_*：只生成补丁计划，不落盘（审批/commit 在图里做）
"""
...
def read_file_range(path: str, start_line: int = 1, end_line: int = 200) -> str:
    """读取文件行区间（start/end 自动 clamp，不会越界崩溃）。"""
...
def grep(pattern: str, path: str = ".", max_results: int = 50) -> str:
    ...
    hits = rt.grep(pat, path, max_results)  # type: ignore[attr-defined]
    return json.dumps({"pattern": pat, "path": path, "count": len(hits), "hits": hits}, ensure_ascii=False)
