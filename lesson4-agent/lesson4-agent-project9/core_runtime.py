# -*- coding: utf-8 -*-
"""
标题：L02-08D core_runtime.py —— 线程级记忆 + Tool Profile（核心仍是写入审批 PREVIEW）
执行代码：
...
# [L02-08D NEW] 更明确的“聊天/代码任务”路由：chat/review/create/tests/implement
...
def submit_impl(final: Optional[str], evidence: Any, task_type: Optional[str]) -> str:
    ...
    if task_type in ("chat", "review", "create", "tests", "implement"):
        tt = task_type  # type: ignore[assignment]

    # chat/review：不强制 submit，但若被调用，作为“软门禁”处理
    if tt in ("chat", "review"):
        has_ro = any(
            tag in "\n".join(STATE.evidence_log)
            for tag in ("[tree]", "[grep]", "[read_range]")
        )
        if not has_ro:
            return _json(
                "REJECT",
                reason=f"{tt} 模式不需要 submit（且缺少只读证据）。请直接用自然语言答复用户。",
            )
