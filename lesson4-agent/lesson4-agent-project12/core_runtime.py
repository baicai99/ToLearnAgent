# -*- coding: utf-8 -*-
"""
标题：L02-08G core_runtime.py —— 运行时 + 写入审批 + pytest 结构化结果
执行代码：
...
class RuntimeState:
    ...
    # [L02-08G NEW] 最后一次 pytest 结果用于 done gate
...
def set_turn_context(user_text: str) -> TaskType:
    ...
    tests_markers = ["pytest", "测试", "test", "让测试通过", "通过测试", "运行测试"]
    implement_markers = [
        "修复",
        "修改",
        "添加",
...
# ===================== [L02-08G NEW] pytest gate 工具实现 =====================
def run_pytest_impl(path: str = "", args: str = "-q") -> str:
    """
    统一用 `python -m pytest`，避免 Windows 下找不到 pytest 可执行文件。
    返回结构化 JSON，且写入 STATE.last_test_* 供 done gate 使用。
    """
    _require_init()
    assert STATE.workdir is not None
    ...
    rel = (path or "").strip()
    extra = (args or "").strip()
    cmd = f"python -m pytest {extra}".strip()
    if rel:
        cmd = f"{cmd} {rel}".strip()
    ...
    # 提取一个非常粗的 summary：最后 10 行 stdout+stderr
    tail = "\n".join(
        (proc.stdout or "").splitlines()[-10:] + (proc.stderr or "").splitlines()[-10:]
    )
