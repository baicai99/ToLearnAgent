# -*- coding: utf-8 -*-
"""
标题：L02-08G graph_agent.py —— pytest gate + 证据链（progress）写入 state
执行代码：
  由 main.py build_graph(...) 后 graph.invoke(...)

本课新增学习内容：
- [L02-08G NEW] tools 节点解析 run_pytest 的结构化结果，写入 state（last_test_rc/summary）
- [L02-08G NEW] “修复/实现/测试”任务：写入后应优先跑 run_pytest（提示模型）
"""
...
class AgentState(TypedDict, total=False):
    ...
    # [L02-08G NEW] pytest 状态（从工具结果写入）
    last_test_rc: Optional[int]
    last_test_cmd: Optional[str]
...
def _make_session_context(state: AgentState) -> str:
    ...
    # [L02-08G NEW] pytest 状态进上下文（让模型知道是否满足 done gate）
    last_rc = state.get("last_test_rc", None)
    last_cmd = state.get("last_test_cmd", "")
    last_sum = (state.get("last_test_summary") or "").strip()
    ...
    return (
        ...
        f"last_rc: {last_rc}\n"
        f"last_cmd: {last_cmd}\n"
        f"last_summary: {last_sum or '(none)'}\n"
        "规则：每一步必须服务于 objective；若涉及修复/实现，写入后优先跑 run_pytest 再判断 done。"
    )
...
def build_graph(
    ...
    *,
    ...
    def router(state: AgentState) -> Dict[str, Any]:
        ...
        if sl in ("help", "/help", "?"):
            reply = "可用命令：new task: <目标> / status / done / abort"
        ...
        if sl in ("status", "/status"):
            ...
            return stop_with(reply, tool_profile="chat", tool_profile_reason="status")
    ...
    def think(state: AgentState) -> Dict[str, Any]:
        ...
        ctx_msg = SystemMessage(content=_make_session_context(state))
        msgs = state.get("messages", [])
    ...
    def tools(state: AgentState) -> Dict[str, Any]:
        ...
        last_test_rc = state.get("last_test_rc", None)
        last_test_cmd = state.get("last_test_cmd", None)
        ...
        for call in tool_calls:
            ...
            if obj and isinstance(obj.get("status"), str):
                ...
                # [L02-08G NEW] PYTEST 结果写入 state，成为 done gate 的证据
                if st == "PYTEST":
                    last_test_rc = obj.get("rc")
                    last_test_cmd = obj.get("cmd")
                    last_test_summary = obj.get("summary") or ""
                    progress.append(f"pytest 已执行：rc={last_test_rc}")
    ...
    def commit(state: AgentState) -> Dict[str, Any]:
        ...
        # [L02-08G NEW] 提示：写入后应优先跑 pytest（对 tests/implement 任务）
        hint = (
            f"[human-approved] commit_patch status={status} patch_id={pid} file={file}\n"
            "提示：如果该任务涉及修复/实现，请下一步调用 run_pytest 进行验证，再决定是否 done。"
        )
        ...
        return {
            ...
            "pending_diff": None,
            "pending_tool": None,
            "last_reply": "补丁已提交。下一步建议运行 run_pytest 验证。",
            "progress": progress,
        }
