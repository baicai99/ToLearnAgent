# -*- coding: utf-8 -*-
"""
标题：L02-08C core_runtime.py —— 写入审批上移到 LangGraph interrupt（工具不再 input）
执行代码：
  由 tools.py / main.py 调用，不直接运行
"""
...
TaskType = Literal["review", "create", "tests", "implement"]
PatchKind = Literal["write_file", "apply_hunks"]


@dataclass
class PendingPatch:
    # [L02-08C NEW] ask 策略下：工具只生成预览，不直接写入；待图层审批后再 commit
    patch_id: str
    file: str  # rel posix
    existed: bool
    kind: PatchKind
...
# -----------------------------
# 任务门禁：写入仅在 create/tests/implement
# -----------------------------


def write_gate() -> Optional[str]:
    if STATE.task_type == "review":
        return "WRITE_NOT_ALLOWED: 当前任务类型为 review（只读审阅）。如需修改/创建文件，请明确说明。"
    return None
...
# -----------------------------
# 写入：ask 策略不写入，只 PREVIEW（由图层 interrupt 审批）
# -----------------------------
...
def write_file_impl(file_path: Optional[str], content: Optional[str]) -> str:
    ...
    if STATE.write_policy == "ask":
        # [L02-08C CHANGE] 不再在工具内 input(y/n)，只生成 PREVIEW，待图层 interrupt 审批
        plan = PendingPatch(
            patch_id=pid,
            file=rel,
            existed=existed,
...
def apply_hunks_impl(file_path: Optional[str], hunks: Any) -> str:
    ...
    if (
        ...
        return _json(
            "ERROR",
            message="hunks 必须是非空 list[dict]，每项包含 start_line/end_line/replacement。创建文件请用 write_file。",
        )
    ...
    if STATE.write_policy == "ask":
        # [L02-08C CHANGE] ask：只 PREVIEW，不写入
        plan = PendingPatch(
            patch_id=pid,
            file=rel,
            existed=True,
...
# [L02-08C NEW] 审批后的 commit / reject（由图层调用）
...
def submit_impl(final: Optional[str], evidence: Any, task_type: Optional[str]) -> str:
    _require_init()

    # [L02-08C NEW] 还有未审批补丁时不允许 submit=ACCEPT（避免“写入没确认就结束”）
    if STATE.pending_patches:
        return _json(
            "REJECT",
            reason="存在未审批的补丁（pending_patches 非空）。请先审批再 submit。",
        )
    ...
    if (
        ...
        return _json("REJECT", reason="evidence must be a non-empty list[str]")

    tt: TaskType = STATE.task_type
    if task_type in ("review", "create", "tests", "implement"):
        tt = task_type

    if tt == "review":
        if STATE.changed_files or STATE.created_files:
            return _json(
                "REJECT", reason="review 任务不应产生写入（changed/created 非空）。"
            )
        has_ro = any(
            tag in "\n".join(STATE.evidence_log)
            for tag in ("[tree]", "[list_files]", "[grep]", "[read_range]")
        )
        if not has_ro:
            return _json(
                "REJECT",
                reason="review 缺少只读证据（tree/list_files/grep/read_range）。",
            )
    ...
    if tt == "create":
        if not STATE.created_files:
            return _json(
                "REJECT", reason="create 未检测到新建文件（created_files 为空）。"
            )
        if not any(any(f in ev for f in STATE.created_files) for ev in evidence):
            return _json(
                "REJECT",
                reason=f"create evidence 未包含新建文件名：{STATE.created_files}",
            )
        return _json("ACCEPT", reason="ok")

    if tt == "tests":
        if STATE.last_test_rc is None:
            return _json("REJECT", reason="tests 未检测到 pytest 运行证据。")
        if STATE.last_test_rc != 0:
            return _json("REJECT", reason=f"pytest 未通过（rc={STATE.last_test_rc}）。")
        has_change = bool(STATE.changed_files or STATE.created_files)
        has_locate = any(
            tag in "\n".join(STATE.evidence_log) for tag in ("[grep]", "[read_range]")
        )
        if not (has_change and has_locate):
            return _json(
                "REJECT",
                reason="tests 证据不足：需要定位证据（grep/read_range）+ 写入（changed/created）。",
            )
        return _json("ACCEPT", reason="ok")

    if not (STATE.changed_files or STATE.created_files):
        return _json(
            "REJECT", reason="implement 未检测到任何写入（changed/created 为空）。"
        )
    if not any(
        (
            "diff" in e
        ...
        return _json("REJECT", reason="implement evidence 未包含变更说明。")
    return _json("ACCEPT", reason="ok")
