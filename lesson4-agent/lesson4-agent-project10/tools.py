# -*- coding: utf-8 -*-
"""
标题：L02-08E tools.py —— @tool 薄封装（参数 Optional，避免缺参崩溃）
执行代码：
...
@tool("repo_tree")
def repo_tree(dir: Optional[str] = ".", recursive: Any = True, max_results: Any = 200) -> str:
    """列出工作区文件结构（只读）。"""
    return cr.repo_tree_impl(dir=dir, recursive=recursive, max_results=max_results)


@tool("list_files")
def list_files(dir: Optional[str] = ".", recursive: Any = False, max_results: Any = 200) -> str:
    """列出目录文件（只读）。"""
    return cr.list_files_impl(dir=dir, recursive=recursive, max_results=max_results)


@tool("grep")
def grep(pattern: Optional[str] = None, path: Optional[str] = ".", max_results: Any = 50) -> str:
    """搜索文本（只读）。"""
    return cr.grep_impl(pattern=pattern, path=path, max_results=max_results)


@tool("read_file_range")
def read_file_range(path: Optional[str] = None, start_line: Any = 1, end_line: Any = 200) -> str:
    """局部读取（越界自动截断）。"""
...
def apply_hunks(file_path: Optional[str] = None, hunks: Any = None) -> str:
    """局部补丁：ask 策略只 PREVIEW（等待图层审批）；allow 直接写。"""
